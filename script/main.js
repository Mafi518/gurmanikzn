...
$('.btn-cart').click(function(){
	...
	var i=-1; $.cart.forEach(function(ct){ i++;
		if(ct){
			var col = $('<div class="cart-col" cart-id="'+i+'">');
			$('<div class="col-img"><img src="'+ct.i+'"></div>').appendTo(col);
			var colName = $('<div class="col-name">'+ct.n+'</div>'); colName.appendTo(col);
			...
			//
			$('<div class="col-count"><input type="number" value="'+ct.c+'"></div>').appendTo(col);
			$('<div class="col-price" price-one="'+ct.p+'">'+(ct.p*ct.c)+'р.</div>').appendTo(col);
			// Удаление
	...
	// Промо-код
	$('<div class="cart-inp"><label>Промо-код:</label><input type="text" name="promo" placeholder="Промо-код" class="w-100"></div>').appendTo(cart);
	...
	// Сумма заказа
	$('<div class="cart-col"><div class="col-price cart-sum big"></div></div>').appendTo(cart);
	$('<input type="hidden" name="budget">').appendTo(cart);
