(function () {
  ...
  var promocodes = [
    {
      ...
      type: "product",
      discount: 20,
      productName: "Классич. ролл с лососем в подарок",
      productImage:
        "https://joinposter.com/upload/pos_cdb_165663/menu/product_1615629410_225.jpg",
    },
    {
      name: "1301",
      type: "product",
      discount: 10,
      productName: "Ролл Жаренный Цезарь в подарок",
      productImage:
        "https://joinposter.com/upload/pos_cdb_165663/menu/product_1615629503_228.jpg",
    },
    {
      name: "16pm",
      type: "percent",
    ...
    $(this)
      ...
      .autocomplete({
        ...
        source: function (request, response) {
          ...
          response($.ui.autocomplete.filter(mas, extractLast(request.term)));
        },
        focus: function () {
  ...
  function getDelivery(data) {
    ...
    // console.log(data);
    $(
      '<div class="delivery-sum">Минимальная сумма заказа: <b>' +
        data.min +
        "</b> руб.</div>"
    ).appendTo(box);
    $(
      '<div class="delivery-sum">Сумма заказа для бесплатной доставки: <b>' +
        data.free +
        "</b> руб.</div>"
    ).appendTo(box);
    ...
    if (data.pay != -1) {
      $(
        '<div class="delivery-sum">Стоимость доставки: <b>' +
          data.pay +
          "</b> руб.</div>"
      ).appendTo(box);
      $(
        '<div class="delivery-sum"><input name="delivery_free" type="hidden" value="' +
          data.free +
          '"></div>'
      ).appendTo(box);
      $(
        '<div class="delivery-sum"><input name="delivery_price" type="hidden" value="' +
          data.pay +
          '"></div>'
      ).appendTo(box);
    }
  ...
  $(".slider-right").click(function () {
    if (slider_current - 1 > -slider_max) slider_current--;
    else slider_current = 0;
    $(".sliders").animate({ "margin-left": slider_current + "00vw" }, 500);
    goSliderInterval();
  });
  $(".slider-left").click(function () {
    if (slider_current + 1 <= 0) slider_current++;
    else slider_current = -slider_max + 1;
    $(".sliders").animate({ "margin-left": slider_current + "00vw" }, 500);
    goSliderInterval();
  });
  ...
  $('<div class="popup-close"></div>')
    .click(function () {
      $(".popup-box").animate({ opacity: "0" }, 300, function () {
        $(this).hide();
      });
  ...
  var updatePrice = function () {
    var price = 0;
    $(".popup-box [price]:checked, .popup-box [price]:selected").each(
      function () {
  ...
  $(".action-add-to-cart").click(function () {
    ...
    if (data.variants && data.variants.length)
      price = data.variants[0]["variant_price"]
        ? data.variants[0]["variant_price"]
        : data.product["price_shop"];
    else price = data.product["price_shop"];
    //
    var cart = $("<div>");
    ...
    $(
      '<div class="product-name" dish-fid="' +
    ...
    $(
      '<div class="product-price" cart-var="price">' + price + " р.</div>"
    ).appendTo(rightBox);
    // select variants
    var selectVariants = $(
      '<div name="variants" cart-check="select_variants">'
    );
    // var selectVariants = $('<select name="variants" cart-var="select_variants">');
    if (data.variants) {
      if (data.variants.length > 1)
        $("<label>Варианты блюда:</label>").appendTo(rightBox);
      else selectVariants.hide();
      var i = 0;
      data.variants.forEach(function (v) {
        ...
        var labelBox = $(
          '<div class="label-box inline select-variants dish-variants" value="' +
            v["variant_id"] +
            '">'
        );
        $(
          '<input id="v' +
            v["variant_id"] +
            '" class="hide" type="radio" name="variants" price="' +
            v["variant_price"] +
            '" variant-fid="' +
            v["variant_fid"] +
            '" ' +
        ...
        $(
          '<label for="v' +
            ...
            '" class="inline">' +
            "<span>" +
            (v["variant_diameter"] > 0
              ? v["variant_diameter"] + " см.<br>"
              : "") +
            (v["variant_price"] > 0
              ? "<b>" + v["variant_price"] + "р.</b><br>"
              : "") +
            (v["variant_weight"] > 0 ? v["variant_weight"] + "гр.<br>" : "") +
            "</span></label>"
        ).appendTo(labelBox);
      ...
      var labelBox = $(
        '<div class="label-box inline select-variants" value="0">'
      );
      $(
        '<input id="v0" class="hide" type="radio" name="variants" price="' +
          data.product["price_shop"] +
          '" checked/>'
      ).appendTo(labelBox);
      $(
    ...
    if (data.group_modifications) {
      if (
        data.product["product_name"].indexOf("Поке") != -1 ||
      ...
      var selectModGroup = $(
        '<div name="group_modifications" cart-check="select_group_modifications">'
      );
      ...
      data.group_modifications[0]["modifications"].forEach(function (v) {
        ...
        var labelBox = $(
          '<div class="label-box select-variants" value="' +
            variant_addon +
            '">'
        );
        $(
          '<label class="select-variants"><input type="' +
            (isAddonsRadio ? "radio" : "checkbox") +
            '" name="group_modifications" price="' +
            price_addon +
            '" variant-fid="' +
            variant_addon +
            '" dish-id="' +
    ...
    if (data.addons_only_pizza) {
      ...
      var selectAddons_only_pizza = $(
        '<div name="addons_only_pizza" cart-check="select_addons_only_pizza">'
      );
      data.addons_only_pizza.forEach(function (v) {
        if (v["variants"]) {
          ...
          $(
            '<label class="select-variants"><input type="checkbox" name="addons_only_pizza" price="' +
              price_addon +
              '" variant-fid="' +
              variant_addon +
              '" dish-id="' +
              ...
              (v["variants"][0]
                ? 'p1="' +
                  v["variants"][0]["variant_price"] +
                  '" f1="' +
                  v["variants"][0]["variant_fid"] +
                  '" '
                : "") +
              (v["variants"][1]
                ? 'p2="' +
                  v["variants"][1]["variant_price"] +
                  '" f2="' +
                  v["variants"][1]["variant_fid"] +
                  '" '
                : "") +
              (v["variants"][2]
                ? 'p3="' +
                  v["variants"][2]["variant_price"] +
                  '" f3="' +
                  v["variants"][2]["variant_fid"] +
                  '" '
                : "") +
              "/><span>" +
    ...
    if (data.addons_pizza) {
      ...
      var selectAddons_pizza = $(
        '<div name="addons_pizza" cart-check="select_addons_pizza">'
      );
      data.addons_pizza.forEach(function (v) {
        if (v["variants"]) {
          ...
          $(
            '<label class="select-variants"><input type="checkbox" name="addons_pizza" price="' +
              price_addon +
              '" dish-id="' +
              v["dish_id"] +
              '"/>' +
    ...
    // select допки к роллам
      ...
      var selectAddons_rolls = $(
        '<div name="addons_rolls" cart-check="select_addons_rolls">'
      );
      data.addons_rolls.forEach(function (v) {
        if (v) {
          ...
          $(
            '<label class="select-variants"><input type="checkbox" name="addons_rolls" price="' +
              price_addon +
              '" dish-id="' +
              v["product_id"] +
              '"/>' +
    ...
    // select допки
      ...
      data.addons.forEach(function (v) {
        if (v["variants"]) {
          ...
          $(
            '<label class="select-variants"><input type="checkbox" name="addons" price="' +
              price_addon +
              '" dish-id="' +
              v["dish_id"] +
              '"/>' +
    ...
    //
    $(
      '<div class="button btn cart-popup-add">Добавить в корзину</div>'
    ).appendTo(rightBox);
    $(
      '<div class="product-desc">' + data.product["ingredients_list"] + "</div>"
    ).appendTo(rightBox);
    ...
    $('[name="variants"] input[type="radio"]').on("change", function () {
      ...
      $(".popup-box .select-variants").each(function () {
        ...
        if (price) {
          ...
          $(this)
            .find("span")
            .html(old_text[0] + " / " + price + " р.");
        }
      });
    });
    $(
      '.popup-box select, .popup-box input[type="checkbox"], .popup-box input[type="radio"]'
  ...
  // === CART INIT === */
  // ==================================================
  if ($.cookie("gurmani_cart")) $.cart = JSON.parse($.cookie("gurmani_cart"));
  else $.cart = [];
  ...
  $(".btn-cart").click(function () {
    var div = $("<div>");
    var cart = $(
      '<form method="POST" class="form-change" action="/order.php">'
    );
    ...
    $.cart.forEach(function (ct) {
      ...
      if (ct) {
        ...
        var colName = $(
          `<div class="col-name col--fix"> <span class="check-name">${ct.n}</span></div>`
        );
        colName.appendTo(col);
        if (ct.d)
          $('<div class="col-desc">' + ct.d + "</div>").appendTo(colName);
        var addons_text = "";
        if (ct.a)
          ct.a.forEach(function (addon) {
            addons_text += "+ " + addon.d + "<br>";
          });
        $('<div class="col-addons">' + addons_text + "</div>").appendTo(
          colName
        );
        //
        $(
          '<div class="col-count"><input class="count" type="number" value="' +
            ct.c +
            '"></div>'
        ).appendTo(col);
        $(
          '<div class="col-price price-change" price-one="' +
            ct.p +
            '">' +
            ct.p * ct.c +
            "р.</div>"
    ...
    $(
      '<div class="cart-col"><div class="col-price cart-sum count-price big total-summ"></div></div>'
    ).appendTo(cart);
    $(
      '<div class="cart-inp"><textarea type="text" name="order_cart" class="hide"></textarea></div>'
    ).appendTo(cart);
    $(
      '<div class="cart-inp"><label>Имя:</label><input type="text" name="name" placeholder="Ваше имя" class="w-100"></div>'
    ).appendTo(cart);
    $(
      '<div class="cart-inp"><label>Телефон:</label><input type="number" name="phone" placeholder="Номер телефона" pattern="[789][0-9]{10}" class="w-100" required></div>'
    ).appendTo(cart);
    $(
      '<div class="cart-inp"><label>Количество персон:</label><select type="number" name="people_amount" placeholder="Количество персон" class="w-100" required><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option><option value="6">6</option></select></div>'
    ).appendTo(cart);
    // Доставка
    var pointsBox = $('<div class="cart-inp"></div>');
    $("<label>Получение заказа:</label>").appendTo(pointsBox);
    $(
      '<label class="select-label "><input type="radio" class="delivery-check affilate-check" name="affiliate" value="3" checked>Доставка по указанному адресу</label>'
    ).appendTo(pointsBox);
    $(
      '<label class="select-label "><input type="radio" class="pickup-check affilate-check" name="affiliate" value="2">Казань, Оренбургский тракт, 8в (самовывоз)</label>'
    ).appendTo(pointsBox);
    ...
    var deliveryInfo = $(
      '<div class="delivery-info quote adress-data">Выберете улицу из списка и здесь появятся условия доставки для вас</div>'
    );
    ...
    for (var i = beginHours; i < 22; i++) {
      if (i > curHours + 1 || curMinutes < 30) {
        if (i >= curHours + 2) {
          $('<option value="' + i + ':00">' + i + ":00</option>").appendTo(
            selectTime
          );
          // console.log(i+':00');
        }
        $('<option value="' + i + ':30">' + i + ":30</option>").appendTo(
          selectTime
        );
        // console.log(i+':30');
      }
    ...
    $(
      '<label class="select-label"><input type="radio" class="cash" name="pay" value="1" checked>Наличными</label>'
    ).appendTo(payType);
    $(
      '<label class="select-label"><input type="radio" class="non-cash" name="pay" value="2">Безналичными</label>'
    ).appendTo(payType);
    payType.appendTo(cart);

    // Промо-код
    $(
      '<div class="cart-inp"><label>Промо-код:</label><input type="text" name="promo" placeholder="Промо-код" class="w-100 promocode_handler"></div>'
    ).appendTo(cart);

    // Комментарий к заказу
    $(
      '<div class="cart-inp"><label>Комментарий к заказу:</label><textarea type="text" name="descr" placeholder="Комментарий..." class="w-100"></textarea></div>'
    ).appendTo(cart);

    // Сумма заказа
    $(
      '<div class="cart-col"><div class="col-price count-price cart-sum big"></div></div>'
    ).appendTo(cart);
    $('<input class="count_summ" type="hidden" name="budget">').appendTo(cart);

    // Кнопка заказать
    $(
      '<div class="cart-inp"><input type="submit" name="submit_order" value="Заказать" class="w-100"></div>'
    ).appendTo(cart);

    // Акция фото в соц. сетях
    $(
      '<div class="quote">Скидки не суммируются и не распространяются на сеты. Подробности уточняйте у операторов</div>'
    ).appendTo(cart);
    $(
      '<a class="" style="color: #ff6807;" href="privacy-policy.php">Политика конфиденциальности</a>'
    ).appendTo(cart);
    ...
    $(".affilate-check").change(checkCart);
    $(".form-change").change(checkCart);

    function checkCart() {
      let countPrice = $(".count_summ").val();
      let delivery = document.querySelector(".delivery-check");
      ...
      function checkedDelivery() {
        if (delivery.checked) {
          ...
          return "pickup";
			...
			document.querySelectorAll('.gift--fix').forEach(element => {
				element.remove()
			});

      function checkInvalidPromocode() {
        if (
          $(".promocode_handler").val() == "700" ||
          $(".promocode_handler").val() == "1301" ||
          $(".promocode_handler").val() == "16pm" ||
          $(".promocode_handler").val() == "WEB" ||
          $(".promocode_handler").val() == "FREE200" ||
          $(".promocode_handler").val() == "НОЧНОЙЖОР" ||
          $(".promocode_handler").val() == "Sum21"
        ) {
        } else {
          $(".promocode_handler").val("");
      ...
      promocodes.find((el) => {
        ...
        // el.name === '16' && hours <= 24 || hours == 1 &&
        // let finalSumDifference = Math.round(countPrice - el.discount)


        if (
          el.name === $(".promocode_handler").val() &&
          el.type === "percent"
        ) {
          if (el.name === "16pm" && checkedDelivery() === "delivery" && hours <= 16 || hours == 12) {
            let finalSumPercent = Math.round(
              countPrice - (countPrice * el.discount) / 100
            );
            $(".count-price").html(`Итого: ${finalSumPercent}р.`);
          } else if (el.name === "16pm" && checkedDelivery() === "pickup" && hours <= 24 || hours == 1) {
            let finalSumPercent = Math.round(
              countPrice - (countPrice * el.discount * 2) / 100
            );
            $(".count-price").html(`Итого: ${finalSumPercent}р.`);
          } else if (el.name === $(".promocode_handler").val()) {
            let finalSumPercent = Math.round(
              countPrice - (countPrice * el.discount) / 100
            )
            $(".count-price").html(`Итого: ${finalSumPercent}р.`);
          ...
          if (el.name === "WEB" && countPrice >= 1000) {
            let finalSumDifference = Math.round(countPrice - el.discount);

            $(".count-price").html(`Итого: ${finalSumDifference}р.`);
          } else if (el.name === "FREE200" && countPrice >= 1200) {
            let finalSumDifference = Math.round(countPrice - el.discount);
          ...
          countPrice >= 600
					...
					if (el.name === "700") {
            $(".count-price").html(`Итого: ${countPrice}р.`);
            document.querySelector('.total-summ').parentNode.insertAdjacentHTML('beforebegin', `
              <div class="cart-col gift--fix">
                <img src="${el.productImage}"
                <div class="col-name col--fix"><span class="check-name">${el.productName}</span></div>
              </div>
            `)
          } else if (el.name === "1301") {
            $(".count-price").html(`Итого: ${countPrice}р.`);
            document.querySelector('.total-summ').parentNode.insertAdjacentHTML('beforebegin', `
            <div class="cart-col gift--fix">
              <img src="${el.productImage}"
...
function timeWarning() {
  if (checkTime <= 22 || checkTime == 10) {
    $('.time-warning').css('display', 'none');
    $('.time-overlay').css('display', 'none')
  } else {
    $('.time-warning').css('display', 'flex')
...
$('.time-warning button').on("click", function() {
...
document.querySelectorAll('.slider-promocode').forEach( function (el) {
  el.addEventListener('click', () => {
    $('.btn-cart').click()
    let promoAttribute = el.getAttribute('data-promocode')
    $('.promocode_handler').val(`${promoAttribute}`)
    $('.promocode_handler').trigger('change')
  })
})
...
setInterval(() => {
  if ($('.cart-count').html() == '0') {
    $('.btn-cart').css('pointer-events', 'none')
  } else {
    $('.btn-cart').css('pointer-events', 'auto')
  }
}, 700);
